...
mkdir -p vina_outputs_tie
mkdir -p vina_console_output_tie
...
for LIGAND in output_ligands_tie/*.pdbqt; do
  ...
  OUTPUT="vina_outputs_tie/${BASENAME}_vina_output.pdbqt"
  
  # Run the vina command
  vina --receptor "$RECEPTOR" --ligand "$LIGAND" --config "$CONFIG" --exhaustiveness="$EXHAUSTIVENESS" --out "$OUTPUT" > vina_console_output_tie/${BASENAME}_energy.txt
